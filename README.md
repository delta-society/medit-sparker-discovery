...
기본 시작은 `/sparker-camp:start`, 직접 진입은 `/sparker-discovery:week2`입니다. 기존 기획 읽기 → 첫 기능 선택 → 환경 확인·실제 구현/실행 → 구조 설명·참가자 변경 선택 → 정상/예외/두 번째 입력 시험 → 재개/선택 제출 준비로 이어집니다. 원본 기획·Week1 기록·Camp 수집 계약을 바꾸지 않습니다. 실제 업로드·업무 성과는 별도입니다. 구현 기록은 별도 `.sparker-implementation`에 저장하며 기존 학습 진행 스키마에 추가하지 않습니다. 상세는 플러그인의 `references/week2-implementation.md`를 따릅니다.
