...
- 1주차 제출은 `/sparker-discovery:week1-submit`, 2주차 구현 결과는 `/sparker-discovery:week2-submit`이다. 기존 `submit`은 주차 라우터이며 모르면 한 번 묻는다. 1주차 제출 ZIP은 기획서만 담는 것이 기본이다. 원문 포함은 별도 명시적 선택이며, **ZIP 구성과 Camp 서버 수집 설정은 별개**다. 파일 준비와 앱 업로드 성공도 구분한다.
