...
def prepare(name, root=None, product=False, linked=False):
    ...
    if linked:
        ...
        q["possible_next_gaps"] = ["기존 목적 KPI 회수 또는 정의·품질·단위·출처·방향 제안/사람 확인", "기존 업무 앱 입력·트리거·결과 레코드", "KPI에 연결할 결과·근거·기록 계기/식별자; 외부 전달처는 선택"]
        q["rules"] += ["제출 업무는 변경 후보이며 KPI를 임의 선택/확정하지 않음", "목표 미확정은 초안 유지; 나머지 설계 진행", "새 폼·대시보드·다운로드보다 기존 앱 재사용 우선", "실제 앱/API 접속 또는 측정 검증을 주장하지 않음"]
    return result
