...
4. **범위 보존.** 선택되지 않은 활용은 `selected_change.non_goals`에 둔다. 선택/제외 답변을 반영하는 리비전에서는 기획서 전체의 `selected_change.change`, AI/코드 역할, 연결·기록 설계, 품질 조건, 수용 시험, 구현 단계를 같은 범위로 다시 맞춘다. 제외된 기능을 non_goals에만 적고 구현·시험에는 남기거나, 사용자의 제외 의미를 더 좁은 뜻으로 바꾸지 않는다. 원문에 있던 기존 작업은 현행 맥락으로 보존할 수 있지만, 그 사실만으로 새 자동화 범위에 포함하지 않는다. 정상/예외 수용 시험은 선택된 활용의 실제 입력→결과→사람 행동을 검증해야 한다. 예를 들어 참조 근거 제공만 선택했다면 그 근거로 검토할 수 있는지 시험하며, 제외된 분석·자동 생성을 새 기능으로 시험하지 않는다. 자료·절차의 축적을 근거로 새 DB·승인 관리·학습 모델·대시보드·자동 수집 기능을 자동 추가하지 않는다. 회사가 정한 범위와 충돌하는 활용은 범위 안 대안으로 제안하고 무단 확장하지 않는다.

**제외한 기능의 결과가 필요한 경우:** 선택한 활용이 제외된 기능의 결과를 참조하더라도, 그 결과는 기존/제공 자료를 읽는 입력으로 두고 생산 기능을 새로 구현하지 않는다. 자료의 존재·접근·품질이 미확인이면 확보 작업으로 남긴다. 데이터가 필요하다는 이유로 제외한 기능을 선행 구현에 되살리지 않는다. 예를 들어 “신규 ST 초안 검토용 참조만, 편차 분석 제외”를 선택하면, 범위는 기존 검토된 공정 기록 읽기→유사 사례 찾기→출처·조건·근거 보여주기→사람의 ST 초안 검토다. 기존 편차값·이상치 표시는 입력 자료나 과거 업무 사실로 보존할 수 있지만 **새 편차 계산·누락/이상치 표시 생성**은 코드 역할·구현 단계·제품 수용 시험에서 제외한다. 정상 시험은 기존 사례의 근거를 실제로 찾고 검토할 수 있는지, 예외 시험은 사례 없음/근거 부족 때 추정·생성을 강행하지 않는지로 작성한다.
